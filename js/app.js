...
(function(){
  ...
  const cards=[...document.querySelectorAll('.guide-match')].map(card=>{
    ...
    return {card,text:norm(text),label:card.dataset.label||'',matchup:card.dataset.matchup||'',time:card.dataset.time||'',court:card.dataset.court||''};
  });
  ...
  function render(){
    ...
    results.innerHTML=`<div class="schedule-results-header"><div><span class="section-tag">Search Results</span><h3>${matches.length} ${matches.length===1?'Match':'Matches'} Found</h3></div><p>Tap a card to locate it on the full board.</p></div><div class="schedule-result-grid">${matches.map((m,i)=>`<button class="match-result-card" type="button" data-match-index="${cards.indexOf(m)}"><div class="match-result-top"><span class="match-time">${m.time||'Time TBA'}</span><span class="match-court">${m.court||'Court TBA'}</span></div><div class="match-division">${m.label}</div><div class="match-teams"><span>${(m.matchup.split(/\s+vs\s+/i)[0]||'TBA')}</span><span class="match-vs">VS</span><span>${(m.matchup.split(/\s+vs\s+/i)[1]||'TBA')}</span></div><small>View on full schedule →</small></button>`).join('')}</div>`;
    results.querySelectorAll('.match-result-card').forEach((btn,idx)=>btn.addEventListener('click',()=>{const target=matches[idx].card;target.scrollIntoView({behavior:'smooth',block:'center',inline:'center'});target.classList.add('search-flash');setTimeout(()=>target.classList.remove('search-flash'),1800);}));
  }
...
(function(){
  ...
  document.querySelectorAll('.fault-card').forEach(d=>entries.push({title:d.querySelector('summary')?.innerText.trim()||'Rule',detail:'Common fault',action:()=>{openTab('rules',true);d.open=true;setTimeout(()=>d.scrollIntoView({behavior:'smooth',block:'center'}),80)}}));
