...
(function(){
  ...
  function render(){
    ...
    results.innerHTML=`<div class="schedule-results-header"><div><span class="section-tag">Search Results</span><h3>${matches.length} ${matches.length===1?'Match':'Matches'} Found</h3></div><p>Results stay here so you can compare matches without losing your place.</p></div><div class="schedule-result-grid">${matches.map((m,i)=>`<article class="match-result-card" data-result-index="${i}"><div class="match-result-top"><span class="match-time">${m.time||'Time TBA'}</span><span class="match-court">${m.court||'Court TBA'}</span></div><div class="match-division">${m.label}</div><div class="match-teams"><span>${(m.matchup.split(/\s+vs\s+/i)[0]||'TBA')}</span><span class="match-vs">VS</span><span>${(m.matchup.split(/\s+vs\s+/i)[1]||'TBA')}</span></div><div class="match-result-actions"><button type="button" class="match-details-toggle" aria-expanded="false">View details</button><button type="button" class="match-view-board">View in full schedule</button></div><div class="match-inline-details" hidden><dl><div><dt>Time</dt><dd>${m.time||'TBA'}</dd></div><div><dt>Court</dt><dd>${m.court||'TBA'}</dd></div><div><dt>Division / Category</dt><dd>${m.label||'TBA'}</dd></div><div><dt>Match</dt><dd>${m.matchup||'TBA'}</dd></div></dl></div></article>`).join('')}</div>`;
    results.querySelectorAll('.match-result-card').forEach((cardEl,idx)=>{
      ...
      detailsBtn?.addEventListener('click',()=>{
        ...
        detailsBtn.textContent=willOpen?'Hide details':'View details';
      });
